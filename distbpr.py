...
def _optimize_partition(user_ratings, prod_mat, nb_prods, l2_reg=0.001, 
    ...
    ratings = ((u_id, prod) for (u_id, (products, _)) in user_ratings for prod in products)

    user_vectors = {u: v for (u, (_, v)) in user_ratings}

    pos_repeated = (a for b in ([x] * negative_samples for x in ratings) for a in b)
  
    neg_ratings = [(x[0], x[1], np.random.randint(nb_prods) + 1) for x in pos_repeated]
    # shuff_ratings = [x for x in neg_ratings]
    ...
    for u_id, pos_id, neg_id in shuff_ratings[:num_samples]:
        ...
        prod_mat[pos_id] += alpha * ((scale * u_vector) - l2_reg * prod_mat[pos_id])
        prod_mat[neg_id] += alpha * ((scale * u_vector) - l2_reg * prod_mat[neg_id])
...
def optimizeMF(ratings, rank=10, nb_iter=10, nb_partitions=4):
    ...
    def vct((u_id, products)): 
        return (u_id, (products, np.random.uniform(size=rank)))

    user_ratings = ratings_by_user.map(vct).persist()
    ...
    for _ in xrange(nb_iter):
        ...
        prod_mat = result.map(lambda x: x[0]).reduce(lambda x, y: x + y) / result.count()
    ...
    user_vectors = map(lambda (u_id, (products, vector)): (u_id, vector), ratings_partitioned.toLocalIterator()) # // try without collect
